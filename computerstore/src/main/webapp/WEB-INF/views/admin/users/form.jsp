...
<div class="card-header bg-body">            <h5 class="fw-bold mb-0">
                <c:choose>
        ...
        <div class="card-body">
            ...
            <form method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/admin/users">
                ...
                <c:if test="${mode != 'create'}">
                    <input type="hidden" name="userId" value="<c:out value='${not empty targetUser ? targetUser.userId : (not empty param.userId ? param.userId : param.id)}'/>"/>
                </c:if>
                    ...
                    <c:otherwise>
                        <div class="mb-3">
                            ...
                            <input type="text" name="username" class="form-control"
                                   value="<c:out value='${not empty formUsername ? formUsername : targetUser.username}'/>"
                                   pattern="[A-Za-z0-9_]{3,30}" required/>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Full name</label>
                            <input type="text" name="fullName" class="form-control"
                                   value="<c:out value='${not empty formFullName ? formFullName : targetUser.fullName}'/>"
                                   maxlength="100" required/>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Email</label>
                            <input type="email" name="email" class="form-control"
                                   value="<c:out value='${not empty formEmail ? formEmail : targetUser.email}'/>"
                                   maxlength="100" required/>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <c:choose>
                                <c:when test="${not empty targetUser && targetUser.role.name() == 'SUPER_ADMIN'}">
                                    <input type="hidden" name="role" value="SUPER_ADMIN"/>
<input type="text" class="form-control bg-body-secondary" value="Super Admin" disabled/>                                    <div class="form-text">The super admin role is managed directly in the database.</div>
                                </c:when>
                                <c:otherwise>
                                    <c:if test="${mode == 'update' && sessionScope.user.userId == param.id}">
                                        <input type="hidden" name="role" value="<c:out value='${selectedRole}'/>"/>
                                    </c:if>
                                    <select name="role" class="form-select"
                                        ...
                                        <c:if test="${sessionScope.user.isSuperAdmin()}">
