...
<c:set var="pageTitle" value="Inbox - TechStore"/>
...
<div class="container py-4">
    <div class="mail-layout">
        <div class="mail-rail">
            ...
            <a class="mail-nav-link active" href="${pageContext.request.contextPath}/mail">
                Inbox
                <span class="mail-nav-count<c:if test="${empty mailCount || mailCount == 0}"> d-none</c:if>">${mailCount}</span>
            </a>
            <a class="mail-nav-link" href="${pageContext.request.contextPath}/mail/sent">Sent</a>
        </div>
        <div class="mail-main">
            <div class="mail-toolbar">
                <div class="btn-group btn-group-sm" role="group" aria-label="Filter inbox">
                    <a class="btn mail-filter${filter == 'all' ? ' active' : ''}" href="${pageContext.request.contextPath}/mail">All</a>
                    <a class="btn mail-filter${filter == 'unread' ? ' active' : ''}" href="${pageContext.request.contextPath}/mail?filter=unread">Unread (${unreadCount})</a>
                    <a class="btn mail-filter${filter == 'read' ? ' active' : ''}" href="${pageContext.request.contextPath}/mail?filter=read">Read (${readCount})</a>
                </div>
            ...
            <div class="mail-list" data-mail-inbox="${pageContext.request.contextPath}/mail/view?id=" data-mail-filter="${filter}">
                <c:forEach var="m" items="${messages}">
                    <a class="mail-row ${not m.readFlag ? 'unread' : ''}" href="${pageContext.request.contextPath}/mail/view?id=${m.messageId}">
                        <span class="mail-avatar-sm">${fn:substring(m.senderName, 0, 1)}</span>
                        <div class="mail-mid">
                            <div class="mail-top">
                                <span class="fw-semibold text-truncate">${m.senderName}</span>
