...
import com.example.computer_store.dao.MailDAO;
...
import com.example.computer_store.model.MailMessage;
import com.example.computer_store.model.User;
import com.example.computer_store.util.EmailUtil;
...
public class MailService {
    ...
    public MailMessage replyTo(int userId, int originalId, String body) {
        ...
        message.setRecipientId(original.getSenderId());
        message.setSubject("Re: " + original.getSubject());
        message.setBody(body.trim());
        mailDAO.create(message);

        notifyByEmail(userService.get(original.getSenderId()), message);
        return message;
    }
